...
import com.smilehat.modules.jasperreports.export.JasperReportsRtfView;
...
public class JasperReportsMultiFormatView extends AbstractJasperReportsView {
	...
	public static final String DEFAULT_HEADERK_KEY = "headers";


	/**
	 * The key of the model parameter that holds the format key.
	 */
	private String headerKey = DEFAULT_HEADERK_KEY;
  ...
  /**
	...
	public JasperReportsMultiFormatView() {
		...
		this.formatMappings.put("rtf", JasperReportsRtfView.class);
	}
	...
	protected void renderReport(JasperPrint populatedReport, Map<String, Object> model, HttpServletResponse response)
		...
		Properties headers = (Properties) model.get(this.headerKey);
		if (headers != null&&!headers.isEmpty()&&headers.containsKey(HEADER_CONTENT_DISPOSITION)) {
			this.getContentDispositionMappings().put(format, headers.get(HEADER_CONTENT_DISPOSITION));
		}	
