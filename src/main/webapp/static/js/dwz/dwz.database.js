...
(function($){
		...
		bringBackSuggest: function(args){
			...
			$box.find(":input").each(function(){
				...
				for (var key in args) {
					...
					if (name == inputName) {
						$input.val(args[key]);
						$input.trigger("changeValidate");
						break;
					}else if (name == inputId) {
						$input.val(args[key]);
						$input.trigger("changeValidate");
						break;
					}
	...
	$.fn.extend({
		lookup: function(){
			return this.each(function(){
				...
				$this.click(function(event){
					...
					if (attrData){
						var attrs = DWZ.jsonEval(attrData);
						options.data={};
						for (var key in attrs) { 
							var _dv=attrs[key];
							if(_dv.startsWith("javascript:")){
								_dv=_dv.replace("javascript:",'');
								options.data[key]=eval(_dv); 
							}else{
								options.data[key]=_dv; 
							}
		...
		multLookupByName: function(){
			return this.each(function(){
				var $this = $(this), args={};
				$this.click(function(event){
					var $unitBox = $this.parents(".unitBox:first");
					$unitBox.find("[name='"+$this.attr("multLookupByName")+"']").each(function(){
						var _args = DWZ.jsonEval($(this).val());
						for (var key in _args) {
							var value = args[key] ? args[key]+"," : "";
							args[key] = value + _args[key];
...
//					});
//					var $unitBox = $this.parents(".unitBox:first");
//					$unitBox.find("[name='"+$this.attr("attachLookup")+"']").each(function(){
//						var _args_id = $(this).val();
//						var _args_path = $(this).parent().find("span.fileName").html();
//						var value = args[$this.attr("attachLookup")] ? args[$this.attr("attachLookup")]+"," : "";
//						args[$this.attr("attachLookup")] = value + _args_id;
//						var path = args[$this.attr("attachLookup")+"_href"] ? args[$this.attr("attachLookup")+"_href"]+"<br/>" : "";
//						args[$this.attr("attachLookup")+"_href"] = path + _args_path;
//						
//					});
//					var $box = _lookup['$target'].parents(".unitBox:first");
//					if ($.isEmptyObject(args)) {
//						$box.find("div.fileViewDiv").html('');
//						$.bringBack({id:'',name:''});
//						return;
//					}
//					$box.find("div.fileViewDiv").html(args[$this.attr("attachLookup")+"_href"]);
//					$.bringBack(args);
//				});
//			});
//		},
			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						validate : $th.attr("validate") || "",
						lookupClass:$th.attr("lookupClass") || "btnLook"
					};
					fields.push(field);
				});
				...
				switch(field.type){
					...
					case 'lookup':
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							...
							+ (field.lookupUrl?('<a class="'+field.lookupClass+'" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" '+attrFrag+' title="查找带回">查找带回</a>'):'');
						break;
//					case 'attach':
//						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
//							+ '<div class="fileViewDiv"></div>'
//							+ '<a class="btnAttach" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" '+' target="dialog" title="查找带回">查找带回</a>';
//						break;
					case 'enum':
						$.ajax({
						...
						break;
					case 'select':
					    html='<select class="combox" name="'+field.name+'" selectedValue="'+field.defaultVal+'" validate="'+field.validate+'" dataUrl="'+field.enumUrl+'"></select>';
					...
					case 'date':
						...
						break;
					default:
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" validate="'+field.validate+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+'/>';
						break;
				}
