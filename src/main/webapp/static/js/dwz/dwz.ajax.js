...
function dialogAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		if (json.navTabId){
			navTab.reload(json.forwardUrl, {navTabId: json.navTabId,data:json.callbackData});
		} else if (json.rel) {
			var $pagerForm = $("#pagerForm", navTab.getCurrentPanel());
			var args = $pagerForm.size()>0 ? $pagerForm.serializeArray() : {}
			navTabPageBreak(args, json.rel);
 ...
 * 上传后预览图片
 */
function uploadifyPreviewComplete(file, data, response){
 ...
 * 上传文件后显示名称和链接
 */
function uploadifyFileComplete(file, data, response){
	var json = DWZ.jsonEval(data); 
	if (json.statusCode == DWZ.statusCode.ok) {
		//${uploadifyFileId}_hiddenId
		//${uploadifyFileId}_picId
		$('#'+this.settings.id+'_hiddenId').val(json.id);
		$('#'+this.settings.id+'_fileId').attr('href',json.downloadUrl);
		$('#'+this.settings.id+'_fileId').html(json.name);
 ...
 * 删除已经上传的文件
 */
function uploadifyFileCompleteDelete(hiddenId,fileId){
     $('#'+hiddenId).val(''); 
	 $('#'+fileId).attr('href', 'javascript:void(0)');
	 $('#'+fileId).html('');
}
 ...
 * 删除已经上传的图片
 */
function uploadifyPreviewCompleteDelete(hiddenId,picId,defaultPicPath){
     $('#'+hiddenId).val('');
     //给一个错误的图片,加载默认图片
