		...
		open:function(url, dlgid, title, options) {
			...
			if(dialog) {
				...
				if(op.fresh || url != $(dialog).data("url")){
					...
					var jDContent = dialog.find(".dialogContent");
					jDContent.loadUrl(url, op.data?op.data:{}, function(){
						jDContent.find("[layoutH]").layoutH(jDContent);
						$(".pageContent", dialog).width($(dialog).width()-14);
...
//					$.pdialog.switchDialog(dialog);
			...
			//修正关闭后清除了当前窗口，造成不能保留之前的数据
			$(dialog).data("fresh", op.fresh);
			//修正可最小化的弹出窗，关闭时阴影不删除的问题
			$(dialog).data("minable", op.minable);
		},
		/**
		 * 切换当前层
		 * @param {Object} dialog
