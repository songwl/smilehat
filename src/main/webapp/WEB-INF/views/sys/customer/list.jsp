...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<%-- <li><a class="add" title="添加用户" rel="sys_customer_new" href="${sctx}/customer/new?navTabId=sys_customer" target="dialog"><span>添加</span></a></li>
			<li><a class="edit" title="编辑用户" rel="sys_customer_update" href="${sctx}/customer/update/{sid}?navTabId=sys_customer" target="dialog" warn="请选择一个用户"><span>编辑</span></a></li> --%>
		...
		<tbody>
			<c:forEach items="${page.content}" var="item" varStatus="index">
				<tr target="sid" rel="${item.id}">
					...
					<td><a title="删除 ${item.user.name}" target="ajaxTodo" href="${sctx}/customer/delete/${item.id}" class="btnDel">删除</a></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
